...
if os.path.exists(EXCEL_FILE):
  ...
  if not today_row.empty:
    ...
    col2.metric(
        'Білки',
        f'{p_val} г',
        delta=f'{p_val - TARGETS["protein"]:.1f} г (ціль {TARGETS["protein"]})',
    )
    col3.metric(
        'Жири',
        f'{f_val} г',
        delta=f'{f_val - TARGETS["fat"]:.1f} г (ціль {TARGETS["fat"]})',
        delta_color='inverse',
    )
    col4.metric(
        'Вуглеводи',
        f'{c_val} г',
        delta=f'{c_val - TARGETS["carbs"]:.1f} г (ціль {TARGETS["carbs"]})',
        delta_color='inverse',
    )
