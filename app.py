...
if rem_kcal > 50:
  if st.button("💡 Що доїсти до норми?", use_container_width=True):
    with st.spinner("Шукаю підходящі варіанти..."):
      ...
      prompt_advice = f"""
            Ти спортивний дієтолог. Поточний час користувача: {current_time_str}.
            Користувачу за день залишилося добрати:
            - Калорії: {rem_kcal:.0f} ккал
            - Білки: {rem_p:.0f} г
            - Жири: {rem_f:.0f} г
            - Вуглеводи: {rem_c:.0f} г

            Запропонуй 2-3 конкретні й прості варіанти продуктів (з назвою та вагою в грамах), щоб закрити саме цей залишок БЖВ.

            КРИТИЧНО ВАЖЛИВА УМОВА (враховуй час {current_time_str}):
            - Якщо зараз пізній вечір або ніч (після 20:00): НЕ пропонуй важку їжу (м'ясо, птицю, крупи, важкі гарніри чи жирні страви). Пропонуй ТІЛЬКИ легкі варіанти для вечора (наприклад: знежирений кисломолочний сир, грецький йогурт, кефір, яєчні білки, легкий протеїновий коктейль).
            - Якщо зараз ранок або день: можна пропонувати повноцінні страви (м'ясо, рибу, каші, горіхи).

            Відповідь надай максимально коротко, маркованим списком.
            """
      try:
